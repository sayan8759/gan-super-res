...
def main(args):
    ...
    # Read original image
    input_tensor = preprocess_one_image(args.inputs_path, False, args.half, device)
    ...
    # Load model weights
    sr_model = load_pretrained_state_dict(sr_model, args.compile_state, args.weights_path)
    print(f"Load `{args.model_arch_name}` model weights `{os.path.abspath(args.weights_path)}` successfully.")
    ...
    cv2.imwrite(args.output_path, cr_image)
    end_time = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path",
                        type=str,
                        default="./output/ESRGAN_x4_baboon.png",
